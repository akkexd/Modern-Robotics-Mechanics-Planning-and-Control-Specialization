...
w1 = np.array([0, 0, 1])
q1 = np.array([-(1 + math.sqrt(3)), 0, 0])
v1 = np.cross(q1, w1)

w2 = np.array([0, 1, 0])
q2 = np.array([-(1 + math.sqrt(3)), 0, -(1 + math.sqrt(3))])
v2 = np.cross(q2, w2)

w3 = np.array([0, 1, 0])
q3 = np.array([-1, 0, (2 + math.sqrt(3))])
v3 = np.cross(q3, w3)
...
with open('BodyScrewAxes_log.txt', 'w') as log_file:
    log_file.write("Body Screw Axes Matrix B:\n")
    log_file.write(np.array2string(B, precision=8, suppress_small=True))
